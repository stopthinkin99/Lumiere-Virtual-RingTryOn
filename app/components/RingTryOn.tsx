    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          ...
          --gold-dk: oklch(50% 0.11 70); --cream: oklch(95% 0.012 82); --muted: oklch(56% 0.014 270);
        }
        ...
        select { background: oklch(18% 0.01 270); color: var(--cream); border: 1px solid oklch(74% 0.12 78 / 0.25); border-radius: 12px; padding: 12px 16px; font-family: 'DM Sans', sans-serif; font-size: 14px; cursor: pointer; outline: none; width: 100%; appearance: none; -webkit-appearance: none; }
        ...
        /* TryOn layout — two-panel, NO scroll, fits full height */
        .tryon-layout {
          display: grid;
        ...
        /* ── MOBILE: keep side-by-side, windowed, no scroll ── */
        @media (max-width: 768px) {
          ...
          /* Nav */
          nav { padding: 12px 16px !important; align-items: center !important; gap: 12px !important; min-height: 80px !important; }
          ...
          .brand-caption { font-size: 8px !important; letter-spacing: .07em !important; white-space: normal !important; line-height: 1.2 !important; }
          ...
          /* Camera */
          .camera-screen { height: calc(100svh - 80px) !important; padding: 10px 14px 14px !important; gap: 10px !important; justify-content: flex-start !important; overflow: hidden !important; }
          .camera-hint { font-size: 9px !important; letter-spacing: .22em !important; margin-top: 2px !important; flex-shrink: 0 !important; }
          .camera-frame { max-height: calc(100svh - 230px) !important; border-radius: 14px !important; flex-shrink: 1 !important; }
          .camera-frame video { max-height: calc(100svh - 230px) !important; object-fit: contain !important; }
          .camera-shot-button { width: 52px !important; height: 52px !important; font-size: 20px !important; flex-shrink: 0 !important; }
          .camera-screen .btn { padding: 10px 16px !important; font-size: 12px !important; flex-shrink: 0 !important; }

          /* ── TRY-ON: side-by-side on mobile, windowed ── */
          .tryon-layout {
          ...
          /* Left panel: windowed — not touching edges */
          .tryon-left {
            margin: 10px 0 10px 10px !important;
            border-radius: 14px !important;
            overflow: hidden !important;
            /* photo fills this windowed area */
          }

          /* Right panel: windowed — not touching edges */
          .tryon-right {
            ...
            border: 1px solid oklch(74% 0.12 78 / 0.12) !important;
          }

          /* Shrink text sizes on mobile right panel */
          .ctrl-label { font-size: 7px !important; margin-bottom: 5px !important; }
          .ctrl-label span { font-size: 9px !important; }
          .swatch-btn { width: 22px !important; height: 22px !important; }
        ...
        /* Compact for small desktop height */
      ...
      <div style={{ minHeight:'100svh', background:'var(--bg)', display:'flex', flexDirection:'column', overflowX:'hidden' }}>
        ...
        <nav style={{ position:'relative', zIndex:50, minHeight:112, padding:'12px 40px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid oklch(74% 0.12 78 / 0.1)', flexShrink:0 }}>
          <div className="serif" style={{ fontSize:50, letterSpacing:'.08em', color:'var(--gold-lt)' }}>Lumière</div>
            ...
            <div className="intro-grid" style={{ position:'relative', zIndex:5, display:'grid', gridTemplateColumns:'0.85fr 1.15fr', alignItems:'center', padding:'0 56px 40px', gap:48, minHeight:'calc(100vh - 88px)' }}>
              <div className="intro-copy" style={{ display:'flex', flexDirection:'column', gap:28, minWidth:0, width:'100%' }}>
                <div style={{ display:'inline-flex', alignItems:'center', gap:12, fontSize:11, letterSpacing:'.38em', textTransform:'uppercase', color:'var(--gold)', animation:'fadeUp .8s .1s ease both' }}>
                  <div style={{ width:30, height:1, background:'var(--gold)' }} />Virtual Try-On
                </div>
                <h1 className="serif" style={{ fontSize:'clamp(48px,5vw,76px)', lineHeight:1.06, letterSpacing:'-.02em', animation:'fadeUp 1s .22s ease both' }}>
                  Virtually Wear the ring<br /><em style={{ color:'var(--gold-lt)' }}>before you Buy</em>
                </h1>
                <p style={{ fontSize:16, fontWeight:300, lineHeight:1.72, color:'var(--muted)', maxWidth:'100%', animation:'fadeUp 1s .38s ease both' }}>Take a photo of your hand.</p>
              ...
              <div className="desktop-only" style={{ position:'relative', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', animation:'fadeIn 1.2s .3s ease both', paddingRight:20 }}>
                ...
                <div style={{ position:'absolute', bottom:16, left:'50%', transform:'translateX(-50%)', display:'flex', gap:6 }}>
                  {[0,1,2].map(i => (
                    <div key={i} onClick={() => setIntroCard(i)} style={{ width:i===introCard?28:8, height:8, borderRadius:4, background:i===introCard?'var(--gold)':'rgba(201,168,50,.25)', transition:'all .4s', cursor:'pointer' }} />
                  ))}
                </div>
              </div>
            </div>
        ...
        {phase === 'camera' && (
          <div className="camera-screen">
            <p className="camera-hint" style={{ fontSize:15, letterSpacing:'.3em', color:'var(--gold-lt)', textTransform:'uppercase', flexShrink:0 }}>
              Hold hand flat · ring finger toward camera
            </p>
            <div className="camera-frame">
              <video ref={videoRef} playsInline muted />
              {!cameraReady && (
                <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(8,7,10,.7)' }}>
                  <div style={{ width:32, height:32, border:'2px solid rgba(201,168,50,.3)', borderTopColor:'var(--gold)', borderRadius:'50%', animation:'spin .8s linear infinite' }} />
                </div>
              )}
            </div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
            <div className="tryon-layout">
              ...
              <div className="tryon-left" ref={containerRef} style={{ cursor:isDragging?'grabbing':'default', touchAction:'none', isolation:'isolate', willChange:'transform' }}>
                {photo && (
                  <img src={photo} alt="hand" draggable={false} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block', pointerEvents:'none', userSelect:'none' }} />
                )}
                <div onMouseDown={onPointerDown} onTouchStart={onPointerDown} style={{ position:'absolute', left:`${ringPos.x}%`, top:`${ringPos.y}%`, transform:`translate(-50%,-50%) rotate(${ringRotation + (selectedRing.rotation ?? 0)}deg)`, cursor:isDragging?'grabbing':'grab', userSelect:'none', touchAction:'none', width:ringSize, willChange:'transform' }}>
              ...
              <div className="tryon-right">
                ...
                <div>
                  ...
                  <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
                    {selectedRing.colors.map(c => (
                      <button key={c.id} onClick={() => setSelectedColorId(c.id)} className={`swatch-btn ${selectedColorId===c.id?'active':''}`} style={{ background:c.swatch }} title={c.label} />
                    ))}
                  </div>
                </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                  <div>
                    <div className="ctrl-label">Ring Size <span>{Math.round(ringSize)}px</span></div>
                    <input type="range" min={20} max={600} value={ringSize} onChange={e => setRingSize(+e.target.value)} />
                  </div>
                  <div>
                    <div className="ctrl-label">Rotate <span>{Math.round(ringRotation)}°</span></div>
