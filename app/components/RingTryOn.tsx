...
const RING_CATALOG = [
  ...
  },
  {
    ...
    colors: [
      ...
      { id: 'yellow',  label: 'Yellow Gold',    swatch: 'linear-gradient(135deg,#e8e8f0,#a0a0b8)', image: '/rings/Yellow_Gold_Emerald.png' },
  ...
  },
  {
    ...
    colors: [
      ...
      { id: 'yellow',     label: 'Yellow Gold',  swatch: 'linear-gradient(135deg,#f0f0f0,#c0c0d0)', image: '/rings/Yellow_Square.png' },
      { id: 'platinum',     label: 'platinum',  swatch: 'linear-gradient(135deg,#f0f0f0,#c0c0d0)', image: '/rings/Platinum_Square.png' },
    ],
  },
  {
    id: 'Oval Love',
  ...
  },
  {
    ...
    colors: [
      ...
      { id: 'rose',     label: 'Rose Gold',  swatch: 'linear-gradient(135deg,#f0f0f0,#c0c0d0)', image: '/rings/Rose_Pear.png' },
      { id: 'yellow',     label: 'Yellow Gold',  swatch: 'linear-gradient(135deg,#f0f0f0,#c0c0d0)', image: '/rings/Yellow_Pear.png' },
    ],
  },
  {
    id: 'vintage',
    ...
    colors: [
      ...
      { id: 'white',    label: 'White Gold', swatch: 'linear-gradient(135deg,#f5e070,#c8a020)', image: '/rings/White_Vintage.png' },
    ],
  },
  {
    id: 'Heart Triumph',
    ...
    colors: [
      ...
      { id: 'white',    label: 'White Gold', swatch: 'linear-gradient(135deg,#f5e070,#c8a020)', image: '/rings/White_Heart.png' },
    ],
  },
]
        ...
        {phase === 'tryOn' && (
          <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
            ...
            <div ref={containerRef}
              style={{ position:'relative', flex:'0 0 auto', height:'clamp(320px, 65vw, 620px)', overflow:'hidden', background:'#000', cursor: isDragging ? 'grabbing' : 'default', touchAction:'none', isolation:'isolate', willChange:'transform' }}>
              ...
              <div onMouseDown={onPointerDown} onTouchStart={onPointerDown}
                style={{ position:'absolute', left:`${ringPos.x}%`, top:`${ringPos.y}%`, transform:`translate(-50%,-50%) rotate(${ringRotation}deg)`, cursor: isDragging ? 'grabbing' : 'grab', userSelect:'none', touchAction:'none', width:ringSize, willChange:'transform', filter:'drop-shadow(0 4px 20px rgba(0,0,0,.6))' }}>
                <img src={selectedColor.image} alt={selectedRing.name} draggable={false} style={{ width:'100%', objectFit:'contain', display:'block', pointerEvents:'none' }} />
              </div>
              {!isDragging && (
                <div style={{ position:'absolute', bottom:12, left:'50%', transform:'translateX(-50%)', background:'rgba(8,7,10,.75)', backdropFilter:'blur(12px)', border:'1px solid oklch(74% 0.12 78 / 0.2)', color:'rgba(240,237,232,.8)', padding:'6px 18px', borderRadius:100, fontSize:11, letterSpacing:'.07em', whiteSpace:'nowrap', pointerEvents:'none' }}>
            ...
            {/* Sliders */}
            <div style={{ background:'oklch(10% 0.008 270)', borderBottom:'1px solid rgba(255,255,255,.05)', padding:'10px 20px', display:'flex', gap:20 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:8, color:'var(--gold)', letterSpacing:'.3em', textTransform:'uppercase', marginBottom:6, fontWeight:600 }}>Size</div>
                <input type="range" min={20} max={600} value={ringSize} onChange={e => setRingSize(+e.target.value)} />
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:8, color:'var(--gold)', letterSpacing:'.3em', textTransform:'uppercase', marginBottom:6, fontWeight:600 }}>Rotate</div>
            ...
            <div style={{ flex:1, overflowY:'auto', background:'oklch(8% 0.008 270)', padding:'16px 16px 0' }}>
              ...
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:9, color:'var(--gold)', letterSpacing:'.3em', textTransform:'uppercase', fontWeight:600, marginBottom:8 }}>Ring Style</div>
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                  {selectedRing.colors.map(c => (
                    <button key={c.id} onClick={() => setSelectedColorId(c.id)} className={`swatch-btn ${selectedColorId === c.id ? 'active' : ''}`}
              ...
              {/* Divider */}
              <div style={{ height:1, background:'linear-gradient(90deg,transparent,oklch(74% 0.12 78 / 0.12),transparent)', margin:'4px 0 14px' }} />

              {/* Action buttons */}
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, paddingBottom:36 }}>
                {/* Save photo */}
                <button onClick={savePhoto}
                  style={{ padding:'14px 0', borderRadius:12, border:'1px solid oklch(74% 0.12 78 / 0.3)', background: photoSaved ? 'oklch(74% 0.12 78 / 0.15)' : 'transparent', color: photoSaved ? 'var(--gold)' : 'var(--cream)', fontSize:12, letterSpacing:'.1em', fontWeight:500, cursor:'pointer', fontFamily:'DM Sans', display:'flex', alignItems:'center', justifyContent:'center', gap:8, transition:'all .3s' }}>
                  {photoSaved ? '✓ Saved!' : (
                    <>
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                      Save Photo
                    </>
                  )}
                </button>
                ...
                <button onClick={addToCart}
                  style={{ padding:'14px 0', borderRadius:12, border:'none', background: addedToCart ? 'var(--gold-dk)' : 'linear-gradient(135deg,var(--gold),var(--gold-lt))', color:'#08070a', fontSize:12, letterSpacing:'.1em', fontWeight:600, cursor:'pointer', fontFamily:'DM Sans', display:'flex', alignItems:'center', justifyContent:'center', gap:8, transition:'all .3s' }}>
                  {addedToCart ? '✓ Added!' : (
                    <>
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
